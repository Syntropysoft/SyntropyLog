...
export const createLevelStyleMap = (c: typeof chalk): LevelStyleMap => ({
  ...
  fatal: {
    level: c.red.bgWhite.bold,
    message: c.red.bold,
    metaKey: c.red.dim,
    metaValue: c.red,
  },
  audit: {
    level: c.magenta.bold,
    message: c.magenta,
    metaKey: c.magenta.dim,
    metaValue: c.magenta,
  },
});
...
export const formatColorfulLog = (
  ...
  const style = styleMap[level as Exclude<LogLevel, 'silent'>] ?? {
    level: c.white.bold,
    message: c.white,
    metaKey: c.gray,
    metaValue: c.white,
...
export class ColorfulConsoleTransport extends BaseConsolePrettyTransport {
  private readonly levelStyleMap: LevelStyleMap;

  constructor(options?: TransportOptions) {
    super(options);
    this.levelStyleMap = createLevelStyleMap(this.chalk);
  }
  ...
  protected formatLogString(logObject: LogEntry): string {
    return formatColorfulLog(logObject, this.chalk, this.levelStyleMap);
  }
}
