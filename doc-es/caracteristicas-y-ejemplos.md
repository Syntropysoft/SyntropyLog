...
**Ejemplo:** No hace falta configurar nada; si instalás el paquete en Linux/Windows/macOS con Node ≥22, el addon se usa automáticamente. Para deshabilitarlo (p. ej. debugging): `logger.disableNativeAddon: true` en `init()` (el paquete no lee variables de entorno).
